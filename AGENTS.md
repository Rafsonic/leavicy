...
- The dialog component (`<EntityFormDialog>`) expects a **non-null** `entity`/`user`
  ...
  // open:
  <Button id="edit-user-button" data-cy="edit-user-button"
    onClick={() => setUserEditDialog(user)}>Edit</Button>
...
best practices** — prefer the harder *recommended* path over the lazy shortcut.
...
```
apps/
  portal/    Employee/company self-service  (port 3100, "SickDesk Portal")
  crm/       HR / company management        (port 3200, "SickDesk CRM")
  central/   Super-admin back-office        (port 3300, "SickDesk Central")
packages/
